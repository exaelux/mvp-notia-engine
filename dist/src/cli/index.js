...
function printFailure(index, kind, errors) {
...
function printSuccess(index, bundle, verbose) {
    if (verbose) {
        console.log(`[${index}] semantic_bundle -> ${bundle.meaning.aggregated_state}`);
        console.log(JSON.stringify(bundle, null, 2));
    }
    else {
        console.log(`[${index}] ${bundle.meaning.aggregated_state.toUpperCase()} | bundle_ref: ${bundle.meaning.bundle_ref}`);
    }
}
async function main() {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (options.anchor) {
            ...
            if (options.verbose) {
                ...
                console.log(`ANCHOR -> ${anchor.network} | tx_id: ${anchor.transaction_id} | ${anchor.status}`);
            }
        }
