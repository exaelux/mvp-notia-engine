...
for (let i = 0; i < totalEvents; i++) {
  ...
  const result: ReturnType<typeof runNotia> = previous
  ...
  if (previous) {
    assert(
      bundle.meaning.derived_from.includes(previous),
      `Derived_from must include previous ref at iteration ${i}`,
    );
  }
...
for (let i = 0; i < totalEvents; i++) {
  ...
  if (previous) {
    assert(
      result.bundle.meaning.derived_from.includes(previous),
      `Mutation scenario: derived_from must include previous ref at ${i}`,
    );
  }
...
assert(
  originalRefs[7] !== bundleRefs[7],
  "Mutation should change bundle_ref at iteration 7",
);
...
assert(
  divergedFromMutationPoint,
  "Mutation should affect chain continuity from iteration 7 onward",
);
